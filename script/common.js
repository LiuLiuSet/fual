	...
	var c = {
		init: function(args){
			...
			api.addEventListener({
				...
				/*判断是否加载中*/
				try{
					if($api.contains($api.dom('#main'), $api.dom('.load-more'))){
						return;
					}
				}catch(e){
				...
				tool.ajax.get({
					...
					if(ct instanceof Array && ct.length!=0){
						$api.append($api.dom('#main'), doT.template($api.html($api.dom('[template=list]')))(ct));
						api.parseTapmode();
					}else{
						api.toast({
							msg: '已经没有更多内容了',
